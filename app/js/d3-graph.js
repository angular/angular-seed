...
function tick() {
    ...
    path.attr('d', function(d) {
        var deltaX = d.target.x - d.source.x,
                ...
                normY = deltaY / dist,
                sourcePadding = 1,
                targetPadding = d.right ? rNode-10 : rNode,
...
//    node = {id:nodes[i].id,x:nodes[i].x,y:nodes[i].y,px:nodes[i].px,py:nodes[i].py};
//     savenodes.push(node);         
//    }
    var savestr = "{\"nodes\":" + JSON.stringify(nodes) + "," + "\"links\":" + JSON.stringify(savelinks) + "}";
//window.open( "data:text/json;charset=utf-8," + savestr)
